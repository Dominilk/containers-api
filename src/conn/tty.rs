...
type TtyReader = Pin<Box<dyn Stream<Item = Result<TtyChunk>> + Send>>;
type TtyWriter = Pin<Box<dyn AsyncWrite + Send>>;
...
impl<'a> AsyncWrite for Multiplexer {
    fn poll_write(
        self: Pin<&mut Self>,
        cx: &mut Context<'_>,
        buf: &[u8],
